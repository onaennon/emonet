...
const i18n = {
  en:{newChat:"New Chat",chats:"Chats",settings:"Settings",del:"Delete",delConfirm:"Delete this chat?",yes:"Yes",no:"No",cancel:"Cancel",
    ...
    helpText1:"EmoNet analyzes your messages to understand your emotional state. As you chat, two neural networks work silently in the background.",
    helpText2:"The colored dot at the top shows your predicted emotional state. Each color represents a different dimension — pleasure (gold), activation (cyan), control (blue), clarity (silver), safety (green), social (pink), and drive (violet). Brighter colors mean stronger emotions.",
    helpText3:"Tap the dot to see detailed charts and graphs of your emotional trajectory over time. Each category can be expanded to see individual emotions and their history.",
    helpText4:"The system learns YOUR unique patterns. Over time it gets better at predicting how your emotions will shift. Early readings have low confidence — this improves with more conversation.",
    helpText5:"Emotion-Aware AI (optional): When enabled, the AI subtly adjusts its tone based on your detected mood. This sends a brief mood summary to the AI model. You can toggle this on/off in the menu.",
    helpText6:"All your data stays on your device. Nothing is stored on any server. If you clear your browser data, your emotional history will be reset."},
  zh:{newChat:"新對話",chats:"對話",settings:"設定",del:"刪除",delConfirm:"確定刪除？",yes:"是",no:"否",cancel:"取消",
    ...
    helpText1:"EmoNet 分析您的訊息以了解您的情緒狀態。聊天時，兩個神經網路在背景中靜靜運作。",
    helpText2:"頂部的彩色圓點顯示您的預測情緒狀態。每種顏色代表不同的維度。更亮的顏色表示更強烈的情緒。",
    helpText3:"點擊圓點可查看詳細的情緒軌跡圖表。每個類別可展開查看個別情緒及其歷史。",
    helpText4:"系統學習您獨特的模式。隨著時間推移，預測會越來越準確。",
    helpText5:"情緒感知 AI（可選）：啟用後，AI 會根據偵測到的情緒微調語氣。可在選單中開關。",
    helpText6:"所有資料保存在您的裝置上。清除瀏覽器資料會重置情緒歷史。"}
};
...
function LineChart({data, lines, colors, height=180, showZero=false, wide=false}) {
  ...
  useEffect(() => {
    ...
    // Draw lines with varying dash patterns to differentiate overlapping lines
    const dashes = [[], [6,3], [2,2], [8,4,2,4], [4,2], [1,3], [10,3]];
    lines.forEach((lk,li) => {
      const col=colors[li];
      ctx.strokeStyle=`rgba(${col[0]},${col[1]},${col[2]},0.9)`;
      ctx.lineWidth = 2;
      ctx.setLineDash(dashes[li % dashes.length] || []);
      ctx.beginPath();
  ...
  return (
    <div style={{height:"100dvh",width:"100vw",display:"flex",flexDirection:"column",background:"#06060c",color:"#efeffa",fontFamily:"'Nunito','Noto Sans TC',sans-serif",overflow:"hidden",position:"relative"}}>
      ...
      {showHelp&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"#06060c",zIndex:46,overflowY:"auto",padding:20}}>
        ...
        {[t.helpText1,t.helpText2,t.helpText3,t.helpText4,t.helpText5,t.helpText6].map((txt,i)=>
          <p key={i} style={{fontSize:15,color:"#b0b0c8",lineHeight:1.7,marginBottom:14}}>{txt}</p>)}
      </div>}
        ...
        <div style={{flex:1,overflowY:"auto",padding:"0 16px 24px",WebkitOverflowScrolling:"touch"}}>
          ...
          {statusTab==="overview"&&<>
            ...
            <div style={{marginBottom:24}}>
              {CD.map((dim,i)=>{const val=dC[i]||0; const pct=(val+1)/2*100;
                return <div key={dim} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",fontSize:14}}>
                  <span style={{width:85,color:"#a0a0b8",textTransform:"capitalize",flexShrink:0}}>{dim}</span>
                  <div style={{flex:1,height:8,background:"#141420",borderRadius:4,overflow:"hidden",position:"relative"}}>
                    <div style={{position:"absolute",left:"50%",top:0,bottom:0,width:1,background:"#2a2a3a"}}/>
                    <div style={{position:"absolute",top:0,bottom:0,left:val>=0?"50%":`${pct}%`,width:`${Math.abs(val)*50}%`,
                      background:`rgba(${CC[dim].join(",")},0.7)`,borderRadius:4}}/></div>
                  <span style={{width:44,textAlign:"right",color:"#9090a8",fontSize:12}}>{val.toFixed(2)}</span></div>;})}
            </div>
            ...
            {/* 3-point trajectory */}
            {traj1&&<div style={{marginBottom:24}}>
              <div style={{fontSize:12,color:"#7a7a9a",letterSpacing:1,marginBottom:8,textTransform:"uppercase"}}>{t.trajectory}</div>
              {[{label:t.prediction,data:traj1,c:conf},{label:"Direction",data:traj2?traj2.map((v,i)=>mood[i]+v):null,c:conf*.65},
                {label:t.attractor,data:traj3,c:conf*.35}].filter(p=>p.data).map((pt,pi)=>
                <div key={pi} style={{marginBottom:10,padding:"8px 12px",background:"#0a0a14",borderRadius:8,border:"1px solid #141420"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    <span style={{fontSize:12,color:"#b0b0c8"}}>{pt.label}</span>
                    <span style={{fontSize:11,color:"#6a6a8a"}}>{(pt.c*100).toFixed(0)}% conf</span></div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{CD.map((d,i)=>
                    <span key={d} style={{fontSize:11,color:`rgba(${CC[d].join(",")},0.8)`,padding:"2px 6px",background:"#0e0e1a",borderRadius:4}}>
                      {d.slice(0,3)} {(pt.data[i]||0).toFixed(2)}</span>)}</div></div>)}
            </div>}
          </>}
